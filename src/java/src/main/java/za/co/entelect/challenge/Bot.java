...
public class Bot {
    ...
    // Mencari jarak pandang ke depan maksimum dari car
    ...
    // Fungsi untuk menentukan prioritas command USE_BOOST
    private int UseBoost(){
        ...
        for(int i=max(myCar.position.block-startBlock+1,0);i<=min(getMaxPos(),myCar.position.block-startBlock+getCurSpeed(0, true));i++){
            if (curLane[i] == null || curLane[i].terrain == Terrain.FINISH) {
                ...
                cntMudOil++;
            }else if(curLane[i].terrain==Terrain.WALL || curLane[i].isOccupiedByCyberTruck){
                cntWallCyber++;
    ...
    // Fungsi untuk menentukan prioritas command Accelerate
    ...
    // Fungsi untuk menentukan prioritas dari command Decelerate
    ...
    // Fungsi untuk menentukan prioritas dari command USE_OIL
    private int UseOil(){
    ...
    // Fungsi untuk menentukan prioritas dari command USE_EMP
    private int UseEMP(){
    ...
    // Fungsi untuk menentukan prioritas dari command USE_LIZARD
    ...
    // Fungsi untuk mengecek apakah ada obstacle di depannya posisinya
    private boolean isNabrakObstacleInfront_atCurrentLane(int lane, int curblock, boolean isBooster, boolean isNextBooster){
        ...
        for (int i = curblock - block + getCurSpeed(0, isBooster) + 1; i <=min(getMaxPos(),curblock - block + getCurSpeed(0, isBooster) + NextSpeed); i++){
            ...
            else if (curLane[i].terrain==Terrain.MUD || curLane[i].terrain==Terrain.OIL_SPILL || curLane[i].terrain==Terrain.WALL || curLane[i].isOccupiedByCyberTruck){
                return true;
            }
        }
        return false;
    ...
    // Fungsi untuk menentukan prioritas dari command TURN_LEFT
    ...
    // Fungsi untuk menentukan prioritas dari command TURN_RIGHT
    ...
    // Fungsi untuk menentukan prioritas dari command FIX
    private int UseFix(){
    ...
    // Fungsi untuk menentukan prioritas dari command DO_NOTHING
